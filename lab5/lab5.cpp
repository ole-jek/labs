...
int main()
{
     ...
     */
    {
        ...
        printf("выбери нужное действие");
        if ((scanf("%d", &choice) == 1) && choice >= 1 && choice <= 4) {
            actions[choice - 1](nArr, nTotal);
        }
        else {
            printf("некорректный ввод действия");
            clearInputBuffer();
        }
    }
    ...
    safeBookFill(dynamicBook->author, "Федор Достоевский", MAX_LENGTH); // dynamicBook->author то же самое что (*dynamicBook).author
    safeBookFill(dynamicBook->title, "Преступление и наказание", MAX_LENGTH);
    dynamicBook->year = 1866;
    dynamicBook->price = 600;
    safeBookFill(dynamicBook->category, "Роман", MAX_LENGTH);
...
void safeBookFill(char* buf, const char* text, int max_size) {
    strcpy(buf, text, max_size - 1); // оставляем место для /0
    buf[max_size - 1] = '\0';
}
...
void inputBook(Book* b) {
    ...
    while (1) {
        printf("категория (Роман, Рассказ, Поэма): ");
        scanf("%39s", b->category);
        if (strcmp(b->category, "Роман") == 0 ||
            strcmp(b->category, "Рассказ") == 0 ||
            strcmp(b->category, "Поэма") == 0) {
            break;
        }
        else {
