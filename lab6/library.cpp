...
void readField(FILE* f, char* buffer, int maxSize) { // функция для деэкранирования
	...
	while (i < maxSize - 1) {
		ch = fgetc(f);
		if (ch == EOF || ch == '\n' || ch == '|') {
			break; // Если встретили чистый разделитель — выходим
		}

		if (ch == '\\') {
			int next = fgetc(f);
			if (next == '|') {
				buffer[i++] = '|'; // экранированный разделитель записывем разделитель как символ
			}
			else if (next == '\\') {
				buffer[i++] = '\\'; // экранированный бэкслеш записываем бэкслэш
			}
			else {
				ungetc(next, f); // если что-то другое то возвращаем в поток
				buffer[i++] = '\\';
			}
		}
		else {
			buffer[i++] = (char)ch; // для обычных символов
		}
	}
	buffer[i] = '\0';
}
